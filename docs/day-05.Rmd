...
```{r}
top_5_county_case <- covid %>% filter(date==max(date)) %>% top_n(5,cases) %>% arrange(-cases)
top_5_county_case
```
...
```{r}
top_5_state_case <- covid %>% filter(date==max(date)) %>% group_by(state) %>% summarize(num_cases=sum(cases)) %>% ungroup() %>%  top_n(5,num_cases) %>% arrange(-num_cases)
top_5_state_case
```
...
```{r}
#Filtering out places with infinite death ratios because it ~should~ be impossible to 0 cases and any deaths
top_5_ratio <- covid %>% filter(date==max(date)) %>% mutate(death_ratio= ((deaths/cases)*100)) %>% filter(death_ratio!=Inf) %>% top_n(5,death_ratio) %>% arrange(-death_ratio)
top_5_ratio
```
...
```{r}
top_5_state_ratio <- covid %>% filter(date==max(date)) %>% group_by(state) %>% summarize(deaths= sum(deaths), cases = sum(cases)) %>% ungroup() %>%
  mutate(death_ratio = (deaths/cases)*100)  %>% top_n(5, death_ratio) %>% arrange(-death_ratio)
top_5_state_ratio
```
